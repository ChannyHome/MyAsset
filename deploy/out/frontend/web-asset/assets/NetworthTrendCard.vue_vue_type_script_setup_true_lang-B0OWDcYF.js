...
const {toDisplayString:_toDisplayString,createElementVNode:_createElementVNode,openBlock:_openBlock,createElementBlock:_createElementBlock,createCommentVNode:_createCommentVNode,normalizeStyle:_normalizeStyle,renderList:_renderList,Fragment:_Fragment,normalizeClass:_normalizeClass,vModelSelect:_vModelSelect,withDirectives:_withDirectives,createTextVNode:_createTextVNode,vShow:_vShow} = await importShared('vue');
...
const _hoisted_12 = {
  key: 2,
  class: "mt-3 rounded-xl border border-slate-200 bg-slate-50/70 px-3 py-3 dark:border-slate-700 dark:bg-slate-950/30"
};
const _hoisted_13 = { class: "flex flex-wrap items-center gap-2 text-xs" };
...
const _sfc_main = /* @__PURE__ */ _defineComponent({
  ...
  props: {
    ...
    range: { default: "3M" },
    bucket: { default: "DAY" },
    rangeStartDate: { default: null },
    rangeEndDate: { default: null },
  ...
  setup(__props, { emit: __emit }) {
    ...
    function currentPlotViewportWidth() {
      ...
      if (props.mode === "PORTFOLIO" || props.mode === "ASSET") {
        ...
        return lines2.map((line, index) => {
          ...
          return {
            ...
            values: labels2.map((label) => valueByLabel.get(label) ?? null)
          };
        });
      }
      const lines = [];
    ...
    const scaledValues = computed(() => {
      ...
      const rows = renderLines.value.flatMap(
        (line) => line.values.slice(bounds.start, bounds.end + 1).filter((value) => value != null && Number.isFinite(value))
      );
      if (rows.length === 0) return [0, 1];
      const min = Math.min(...rows);
      const max = Math.max(...rows);
      if (min === max) {
        const padding2 = Math.max(Math.abs(min) * 0.1, isAmountAxis.value ? 1e3 : 1);
        return [min - padding2, max + padding2];
      }
      const padding = Math.max((max - min) * 0.1, isAmountAxis.value ? 1e3 : 0.5);
    ...
    function setZoomLevel(next) {
      ...
      zoomLevel.value = Math.max(-2, Math.min(2, next));
    ...
    function measurePlotViewport(keepLatest = false) {
    ...
    function updatePlotViewportWidth() {
      const el = plotScrollRef.value;
      if (!el) return false;
    ...
    return (_ctx, _cache) => {
      return _openBlock(), _createElementBlock("article", _hoisted_1, [
        ...
        !expanded.value ? (_openBlock(), _createElementBlock("div", _hoisted_10, [
          ...
          chartLabels.value.length > 0 ? (_openBlock(), _createElementBlock("p", _hoisted_11, " Range: " + _toDisplayString(rangeMetaText.value || `${firstPoint.value?.label ?? "-"} -> ${lastPoint.value?.label ?? "-"}`), 1)) : _createCommentVNode("", true)
        ])) : _createCommentVNode("", true),
          ...
          _createElementVNode("div", _hoisted_13, [
            ...
            (_openBlock(), _createElementBlock(_Fragment, null, _renderList(rangeOptions, (item) => {
              return _createElementVNode("button", {
                ...
                class: _normalizeClass([
                  ...
                  rangeModel.value === item.key ? "border-indigo-400 bg-indigo-100 text-indigo-700 dark:bg-indigo-900/40 dark:text-indigo-200" : "border-slate-300 text-slate-700 hover:bg-slate-100 dark:border-slate-700 dark:text-slate-200 dark:hover:bg-slate-800"
                ]),
            ...
            (_openBlock(), _createElementBlock(_Fragment, null, _renderList(bucketOptions, (item) => {
              return _createElementVNode("button", {
                ...
                class: _normalizeClass([
                  "rounded-lg border px-3 py-1.5 font-semibold transition-colors",
                  bucketModel.value === item.key ? "border-indigo-400 bg-indigo-100 text-indigo-700 dark:bg-indigo-900/40 dark:text-indigo-200" : "border-slate-300 text-slate-700 hover:bg-slate-100 dark:border-slate-700 dark:text-slate-200 dark:hover:bg-slate-800"
                ]),
                onClick: ($event) => bucketModel.value = item.key
              }, _toDisplayString(item.label), 11, _hoisted_15);
            }), 64))
          ]),
          rangeMetaText.value ? (_openBlock(), _createElementBlock("p", _hoisted_16, " Range anchor: latest snapshot - " + _toDisplayString(rangeMetaText.value), 1)) : _createCommentVNode("", true)
        ])) : _createCommentVNode("", true),
        expanded.value && __props.showModeToggle ? (_openBlock(), _createElementBlock("div", _hoisted_17, [
          _createElementVNode("button", {
            type: "button",
            class: _normalizeClass([
          ...
          _createElementVNode("div", _hoisted_33, [
            ...
            _createElementVNode("div", _hoisted_36, [
              ...
              _createElementVNode("button", {
                type: "button",
                class: _normalizeClass([
                  "rounded-lg border px-2 py-1 text-xs font-semibold transition-colors disabled:cursor-not-allowed disabled:opacity-40",
                  zoomLevel.value <= -2 ? "border-slate-300 text-slate-500 dark:border-slate-700 dark:text-slate-500" : "border-slate-300 text-slate-700 hover:bg-slate-100 dark:border-slate-700 dark:text-slate-200 dark:hover:bg-slate-800"
                ]),
              ...
              _createElementVNode("button", {
                type: "button",
                class: _normalizeClass([
                  "rounded-lg border px-2.5 py-1 text-xs font-semibold transition-colors disabled:cursor-not-allowed disabled:opacity-40",
                  zoomLevel.value === 0 ? "border-slate-300 text-slate-500 dark:border-slate-700 dark:text-slate-500" : "border-indigo-300 text-indigo-700 hover:bg-indigo-50 dark:border-indigo-700 dark:text-indigo-200 dark:hover:bg-indigo-950/40"
                ]),
                ...
                (_openBlock(true), _createElementBlock(_Fragment, null, _renderList(ticks.value, (tick) => {
                  return _openBlock(), _createElementBlock("span", {
                    ...
                    style: _normalizeStyle({
                      ...
                      filter: props.maskAmounts && isAmountAxis.value ? "blur(6px)" : void 0
                    })
                  }, _toDisplayString(formatAxisValue(tick.value)), 5);
                }), 128))
              ]),
              _createElementVNode("div", {
                ref_key: "plotScrollRef",
                ref: plotScrollRef,
                class: "overflow-x-auto",
                onScroll: syncVisibleRange
              }, [
                (_openBlock(), _createElementBlock("svg", {
                  viewBox: `0 0 ${plotWidth.value} ${chartHeight}`,
                  class: "h-[320px] bg-slate-50 dark:bg-slate-950/40 md:h-[380px] xl:h-[420px]",
                  style: _normalizeStyle({ width: svgWidthStyle.value, minWidth: svgWidthStyle.value })
                }, [
