import { importShared } from './__federation_fn_import-B1auV5c8.js';
import { u as useUiStore } from './ui-DfQNiIxJ.js';

const {defineComponent:_defineComponent$3} = await importShared('vue');

const {toDisplayString:_toDisplayString$3,createElementVNode:_createElementVNode$3,openBlock:_openBlock$3,createElementBlock:_createElementBlock$3,createCommentVNode:_createCommentVNode$3,normalizeStyle:_normalizeStyle$3,renderList:_renderList$2,Fragment:_Fragment$2,normalizeClass:_normalizeClass$2} = await importShared('vue');

const _hoisted_1$3 = { class: "rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900" };
const _hoisted_2$3 = { class: "flex items-start justify-between gap-2" };
const _hoisted_3$3 = { class: "flex items-center gap-2" };
const _hoisted_4$3 = { class: "text-sm font-semibold text-slate-900 dark:text-slate-100" };
const _hoisted_5$3 = ["aria-pressed", "aria-label"];
const _hoisted_6$3 = {
  key: 0,
  class: "mt-0.5 text-xs text-slate-500 dark:text-slate-400"
};
const _hoisted_7$3 = { class: "flex items-center gap-2" };
const _hoisted_8$3 = { class: "rounded-full bg-slate-100 px-2 py-0.5 text-[11px] text-slate-600 dark:bg-slate-800 dark:text-slate-300" };
const _hoisted_9$3 = {
  key: 0,
  class: "mt-4 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-xs text-slate-600 dark:border-slate-700 dark:bg-slate-800/70 dark:text-slate-300"
};
const _hoisted_10$3 = {
  key: 1,
  class: "mt-4 rounded-xl border border-slate-200 bg-slate-50 px-3 py-3 text-sm text-slate-600 dark:border-slate-700 dark:bg-slate-800/70 dark:text-slate-300"
};
const _hoisted_11$3 = { class: "mt-1" };
const _hoisted_12$3 = {
  key: 2,
  class: "mt-4 rounded-xl bg-slate-100 p-3 text-xs text-slate-500 dark:bg-slate-800 dark:text-slate-300"
};
const _hoisted_13$3 = {
  key: 3,
  class: "mt-4 rounded-xl bg-rose-50 p-3 text-xs text-rose-700 dark:bg-rose-950/30 dark:text-rose-200"
};
const _hoisted_14$3 = {
  key: 4,
  class: "mt-4 grid min-h-[180px] grid-cols-1 items-start gap-3 md:grid-cols-[192px_minmax(0,1fr)]"
};
const _hoisted_15$3 = { class: "relative mx-auto h-[11.5rem] w-[11.5rem] md:h-[12rem] md:w-[12rem]" };
const _hoisted_16$3 = ["data-donut-stops", "data-donut-start-angle"];
const _hoisted_17$3 = { class: "absolute left-1/2 top-1/2 flex h-[6rem] w-[6rem] -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full border border-slate-200 bg-white px-1 text-center text-[12px] font-semibold text-slate-700 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200" };
const _hoisted_18$3 = ["onClick"];
const _hoisted_19$3 = { class: "block max-w-[8.6rem] truncate leading-tight text-slate-700 md:max-w-none md:break-words dark:text-slate-200" };
const _hoisted_20$3 = { class: "shrink-0 font-semibold text-slate-700 dark:text-slate-200" };
const _hoisted_21$1 = {
  key: 0,
  class: "absolute left-2 right-2 top-full z-20 mt-1 rounded-md border border-slate-300 bg-slate-950/95 px-2 py-1 text-[11px] text-slate-100 shadow-lg"
};
const _hoisted_22$1 = {
  key: 0,
  class: "rounded-lg bg-slate-50 px-2 py-2 text-xs text-slate-500 dark:bg-slate-800 dark:text-slate-400"
};
const {computed: computed$3,onBeforeUnmount,onMounted: onMounted$3,ref: ref$3,watch: watch$3} = await importShared('vue');

const {storeToRefs: storeToRefs$1} = await importShared('pinia');
const _sfc_main$3 = /* @__PURE__ */ _defineComponent$3({
  __name: "AllocationDonutCard",
  props: {
    title: {},
    subtitle: { default: "" },
    currency: {},
    total: {},
    items: {},
    startPosition: { default: "TOP" },
    maskAmounts: { type: Boolean, default: false },
    loading: { type: Boolean, default: false },
    error: { default: "" },
    mobileTopN: {},
    enableMobileTopN: { type: Boolean, default: true },
    storageKey: { default: "" }
  },
  setup(__props) {
    const props = __props;
    const uiStore = useUiStore();
    const { mobileAllocationTopN } = storeToRefs$1(uiStore);
    const expanded = ref$3(true);
    const infoOpen = ref$3(false);
    const palette = [
      "#0ea5e9",
      "#10b981",
      "#f59e0b",
      "#8b5cf6",
      "#ef4444",
      "#14b8a6",
      "#ec4899",
      "#a3e635",
      "#f97316",
      "#6366f1"
    ];
    const normalizedItems = computed$3(
      () => props.items.filter((item) => Number.isFinite(item.value) && item.ratioPct > 0).map((item, index) => ({
        ...item,
        color: palette[index % palette.length]
      }))
    );
    const viewportWidth = ref$3(typeof window !== "undefined" ? window.innerWidth : 1280);
    const activeLegendTooltipKey = ref$3(null);
    function loadExpandedState() {
      if (typeof window === "undefined" || !props.storageKey) return;
      const raw = window.localStorage.getItem(props.storageKey);
      if (raw === "1") expanded.value = true;
      if (raw === "0") expanded.value = false;
    }
    function updateViewportWidth() {
      viewportWidth.value = window.innerWidth;
      if (viewportWidth.value >= 768) {
        activeLegendTooltipKey.value = null;
      }
    }
    onMounted$3(() => {
      loadExpandedState();
      if (typeof uiStore.init === "function") {
        uiStore.init();
      }
      window.addEventListener("resize", updateViewportWidth, { passive: true });
    });
    onBeforeUnmount(() => {
      window.removeEventListener("resize", updateViewportWidth);
    });
    watch$3(expanded, (value) => {
      if (typeof window === "undefined" || !props.storageKey) return;
      window.localStorage.setItem(props.storageKey, value ? "1" : "0");
    });
    const listScrollThreshold = computed$3(() => {
      if (viewportWidth.value >= 1536) return 16;
      if (viewportWidth.value >= 1280) return 14;
      if (viewportWidth.value >= 1024) return 12;
      if (viewportWidth.value >= 768) return 10;
      return 8;
    });
    const isMobileViewport = computed$3(() => viewportWidth.value < 768);
    const effectiveMobileTopN = computed$3(() => {
      const fromProp = Number(props.mobileTopN);
      if (Number.isFinite(fromProp) && fromProp > 0) {
        return Math.min(12, Math.max(1, Math.trunc(fromProp)));
      }
      return Math.min(12, Math.max(1, Math.trunc(Number(mobileAllocationTopN.value || 6))));
    });
    const displayItems = computed$3(() => {
      const rows = normalizedItems.value;
      const topN = effectiveMobileTopN.value;
      if (!props.enableMobileTopN || !isMobileViewport.value || rows.length <= topN) {
        return rows;
      }
      const head = rows.slice(0, topN);
      const tail = rows.slice(topN);
      const othersValue = tail.reduce((sum, item) => sum + item.value, 0);
      const othersRatio = tail.reduce((sum, item) => sum + item.ratioPct, 0);
      if (othersRatio <= 0) {
        return head;
      }
      return [
        ...head,
        {
          key: "__mobile_others__",
          label: "Others",
          value: othersValue,
          ratioPct: othersRatio,
          color: "#64748b"
        }
      ];
    });
    const topDisplayItem = computed$3(() => displayItems.value[0] ?? null);
    const collapsedSummary = computed$3(() => {
      if (displayItems.value.length === 0 || !topDisplayItem.value) {
        return "No allocation data.";
      }
      return `Top allocation: ${topDisplayItem.value.label} ${formatPercent(topDisplayItem.value.ratioPct)}`;
    });
    const shouldEnableListScroll = computed$3(() => displayItems.value.length > listScrollThreshold.value);
    const startAngle = computed$3(() => {
      if (props.startPosition === "RIGHT") return 90;
      if (props.startPosition === "LEFT") return 270;
      return 0;
    });
    const donutExportStops = computed$3(
      () => displayItems.value.map((item) => `${Math.max(0, Math.min(100, item.ratioPct)).toFixed(6)}:${item.color}`).join("|")
    );
    const donutStyle = computed$3(() => {
      if (displayItems.value.length === 0) {
        return { background: "conic-gradient(#334155 0% 100%)" };
      }
      let cursor = 0;
      const stops = displayItems.value.map((item) => {
        const ratio = Math.max(0, Math.min(100, item.ratioPct));
        const start = cursor;
        const end = Math.min(100, cursor + ratio);
        cursor = end;
        return `${item.color} ${start.toFixed(3)}% ${end.toFixed(3)}%`;
      }).join(", ");
      return { background: `conic-gradient(from ${startAngle.value}deg, ${stops})` };
    });
    function toggleLegendTooltip(key) {
      if (!isMobileViewport.value) return;
      activeLegendTooltipKey.value = activeLegendTooltipKey.value === key ? null : key;
    }
    function formatCurrency(value, currency) {
      return new Intl.NumberFormat("ko-KR", {
        style: "currency",
        currency,
        maximumFractionDigits: 0
      }).format(value || 0);
    }
    function formatPercent(value) {
      return `${value.toFixed(2)}%`;
    }
    return (_ctx, _cache) => {
      return _openBlock$3(), _createElementBlock$3("article", _hoisted_1$3, [
        _createElementVNode$3("div", _hoisted_2$3, [
          _createElementVNode$3("div", null, [
            _createElementVNode$3("div", _hoisted_3$3, [
              _createElementVNode$3("h3", _hoisted_4$3, _toDisplayString$3(__props.title), 1),
              _createElementVNode$3("button", {
                type: "button",
                class: "inline-flex h-7 w-7 items-center justify-center rounded-full border border-slate-300 text-xs font-semibold text-slate-700 transition-colors hover:bg-slate-100 dark:border-slate-700 dark:text-slate-200 dark:hover:bg-slate-800",
                "aria-pressed": infoOpen.value,
                "aria-label": infoOpen.value ? "Hide allocation info" : "Show allocation info",
                onClick: _cache[0] || (_cache[0] = ($event) => infoOpen.value = !infoOpen.value)
              }, " i ", 8, _hoisted_5$3)
            ]),
            __props.subtitle ? (_openBlock$3(), _createElementBlock$3("p", _hoisted_6$3, _toDisplayString$3(__props.subtitle), 1)) : _createCommentVNode$3("", true)
          ]),
          _createElementVNode$3("div", _hoisted_7$3, [
            _createElementVNode$3("span", _hoisted_8$3, _toDisplayString$3(__props.currency), 1),
            _createElementVNode$3("button", {
              type: "button",
              class: "rounded-xl border border-slate-300 px-3 py-2 text-sm font-semibold text-slate-700 transition-colors hover:bg-slate-100 dark:border-slate-700 dark:text-slate-200 dark:hover:bg-slate-800",
              onClick: _cache[1] || (_cache[1] = ($event) => expanded.value = !expanded.value)
            }, _toDisplayString$3(expanded.value ? "Collapse" : "Expand"), 1)
          ])
        ]),
        infoOpen.value ? (_openBlock$3(), _createElementBlock$3("div", _hoisted_9$3, [..._cache[2] || (_cache[2] = [
          _createElementVNode$3("p", null, "This donut shows the latest allocation mix for the selected scope.", -1),
          _createElementVNode$3("p", { class: "mt-1" }, "Use it to spot the biggest positions quickly. On mobile, smaller slices can be grouped into Others.", -1)
        ])])) : _createCommentVNode$3("", true),
        !expanded.value ? (_openBlock$3(), _createElementBlock$3("div", _hoisted_10$3, [
          _cache[3] || (_cache[3] = _createElementVNode$3("p", { class: "font-medium text-slate-700 dark:text-slate-200" }, "Collapsed. Click Expand to preview the latest allocation mix.", -1)),
          _createElementVNode$3("p", _hoisted_11$3, _toDisplayString$3(collapsedSummary.value), 1),
          _createElementVNode$3("p", {
            class: "mt-1",
            style: _normalizeStyle$3(props.maskAmounts ? { filter: "blur(6px)" } : void 0)
          }, " Total " + _toDisplayString$3(formatCurrency(__props.total, __props.currency)), 5)
        ])) : __props.loading ? (_openBlock$3(), _createElementBlock$3("div", _hoisted_12$3, " Loading... ")) : __props.error ? (_openBlock$3(), _createElementBlock$3("div", _hoisted_13$3, _toDisplayString$3(__props.error), 1)) : (_openBlock$3(), _createElementBlock$3("div", _hoisted_14$3, [
          _createElementVNode$3("div", _hoisted_15$3, [
            _createElementVNode$3("div", {
              class: "absolute inset-0 rounded-full shadow-[inset_0_0_0_1px_rgba(15,23,42,0.2)]",
              style: _normalizeStyle$3(donutStyle.value),
              "data-export-donut": "1",
              "data-donut-stops": donutExportStops.value,
              "data-donut-start-angle": String(startAngle.value)
            }, null, 12, _hoisted_16$3),
            _createElementVNode$3("div", _hoisted_17$3, [
              _createElementVNode$3("span", {
                style: _normalizeStyle$3(props.maskAmounts ? { filter: "blur(6px)" } : void 0)
              }, _toDisplayString$3(formatCurrency(__props.total, __props.currency)), 5)
            ])
          ]),
          _createElementVNode$3("ul", {
            class: _normalizeClass$2([
              "space-y-1 pr-1",
              shouldEnableListScroll.value ? "max-h-[11rem] overflow-y-auto md:max-h-[11rem] lg:max-h-[12rem] xl:max-h-[13rem]" : ""
            ])
          }, [
            (_openBlock$3(true), _createElementBlock$3(_Fragment$2, null, _renderList$2(displayItems.value, (item) => {
              return _openBlock$3(), _createElementBlock$3("li", {
                key: item.key,
                class: "relative flex items-center justify-between gap-2 rounded-lg bg-slate-50 px-2 py-1 text-xs dark:bg-slate-800"
              }, [
                _createElementVNode$3("button", {
                  type: "button",
                  class: "flex min-w-0 items-center gap-2 text-left",
                  onClick: ($event) => toggleLegendTooltip(item.key)
                }, [
                  _createElementVNode$3("span", {
                    class: "h-2.5 w-2.5 rounded-full",
                    style: _normalizeStyle$3({ backgroundColor: item.color })
                  }, null, 4),
                  _createElementVNode$3("span", _hoisted_19$3, _toDisplayString$3(item.label), 1)
                ], 8, _hoisted_18$3),
                _createElementVNode$3("span", _hoisted_20$3, _toDisplayString$3(formatPercent(item.ratioPct)), 1),
                isMobileViewport.value && activeLegendTooltipKey.value === item.key ? (_openBlock$3(), _createElementBlock$3("div", _hoisted_21$1, _toDisplayString$3(item.label), 1)) : _createCommentVNode$3("", true)
              ]);
            }), 128)),
            displayItems.value.length === 0 ? (_openBlock$3(), _createElementBlock$3("li", _hoisted_22$1, " No allocation data. ")) : _createCommentVNode$3("", true)
          ], 2)
        ]))
      ]);
    };
  }
});

const {defineComponent:_defineComponent$2} = await importShared('vue');

const {toDisplayString:_toDisplayString$2,createElementVNode:_createElementVNode$2,openBlock:_openBlock$2,createElementBlock:_createElementBlock$2,createCommentVNode:_createCommentVNode$2,renderList:_renderList$1,Fragment:_Fragment$1,createTextVNode:_createTextVNode$2,normalizeStyle:_normalizeStyle$2} = await importShared('vue');

const _hoisted_1$2 = { class: "rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900" };
const _hoisted_2$2 = { class: "flex items-start justify-between gap-2" };
const _hoisted_3$2 = { class: "flex items-center gap-2" };
const _hoisted_4$2 = { class: "text-sm font-semibold text-slate-900 dark:text-slate-100" };
const _hoisted_5$2 = ["aria-pressed", "aria-label"];
const _hoisted_6$2 = {
  key: 0,
  class: "mt-0.5 text-xs text-slate-500 dark:text-slate-400"
};
const _hoisted_7$2 = { class: "flex items-center gap-2" };
const _hoisted_8$2 = { class: "rounded-full bg-slate-100 px-2 py-0.5 text-[11px] text-slate-600 dark:bg-slate-800 dark:text-slate-300" };
const _hoisted_9$2 = {
  key: 0,
  class: "mt-4 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-xs text-slate-600 dark:border-slate-700 dark:bg-slate-800/70 dark:text-slate-300"
};
const _hoisted_10$2 = {
  key: 1,
  class: "mt-4 rounded-xl bg-slate-100 p-3 text-xs text-slate-500 dark:bg-slate-800 dark:text-slate-300"
};
const _hoisted_11$2 = {
  key: 2,
  class: "mt-4 rounded-xl bg-rose-50 p-3 text-xs text-rose-700 dark:bg-rose-950/30 dark:text-rose-200"
};
const _hoisted_12$2 = {
  key: 3,
  class: "mt-4 rounded-xl bg-slate-100 p-3 text-xs text-slate-500 dark:bg-slate-800 dark:text-slate-300"
};
const _hoisted_13$2 = {
  key: 4,
  class: "mt-4 rounded-xl border border-slate-200 bg-slate-50 px-3 py-3 text-sm text-slate-600 dark:border-slate-700 dark:bg-slate-800/70 dark:text-slate-300"
};
const _hoisted_14$2 = { class: "mt-1" };
const _hoisted_15$2 = {
  key: 0,
  class: "mt-1"
};
const _hoisted_16$2 = {
  key: 5,
  class: "mt-4 space-y-3"
};
const _hoisted_17$2 = { class: "flex min-h-[180px] flex-wrap gap-1.5 rounded-xl bg-slate-950/95 p-1.5" };
const _hoisted_18$2 = { class: "truncate font-semibold" };
const _hoisted_19$2 = { class: "text-[11px] text-white/90" };
const _hoisted_20$2 = {
  key: 0,
  class: "text-[11px] text-slate-500 dark:text-slate-400"
};
const {computed: computed$2,onMounted: onMounted$2,ref: ref$2,watch: watch$2} = await importShared('vue');

const _sfc_main$2 = /* @__PURE__ */ _defineComponent$2({
  __name: "AllocationTreemapCard",
  props: {
    title: {},
    subtitle: { default: "" },
    currency: {},
    items: {},
    maskAmounts: { type: Boolean, default: false },
    loading: { type: Boolean, default: false },
    error: { default: "" },
    storageKey: { default: "" }
  },
  setup(__props) {
    const props = __props;
    function normalize(min, max, value) {
      const denom = max - min;
      if (!Number.isFinite(denom) || denom === 0) {
        return 1;
      }
      return Math.max(0, Math.min(1, (value - min) / denom));
    }
    function colorForReturn(returnPct, range) {
      if (returnPct == null || !Number.isFinite(returnPct)) {
        return "hsl(215, 14%, 38%)";
      }
      if (returnPct === 0) {
        return "hsl(215, 14%, 38%)";
      }
      if (!range.hasRange) {
        return returnPct > 0 ? "hsl(145, 68%, 32%)" : "hsl(2, 70%, 36%)";
      }
      if (returnPct > 0) {
        const intensity2 = range.max <= 0 ? 0 : range.min >= 0 ? normalize(range.min, range.max, returnPct) : normalize(0, range.max, returnPct);
        const saturation2 = 52 + intensity2 * 34;
        const lightness2 = 44 - intensity2 * 16;
        return `hsl(145, ${saturation2}%, ${lightness2}%)`;
      }
      const intensity = range.min >= 0 ? 0 : range.max <= 0 ? 1 - normalize(range.min, range.max, returnPct) : 1 - normalize(range.min, 0, returnPct);
      const saturation = 54 + intensity * 32;
      const lightness = 46 - intensity * 16;
      return `hsl(2, ${saturation}%, ${lightness}%)`;
    }
    const returnRange = computed$2(() => {
      const values = props.items.map((item) => item.returnPct).filter((value) => value != null && Number.isFinite(value));
      if (values.length === 0) {
        return { hasRange: false, min: 0, max: 0 };
      }
      return {
        hasRange: true,
        min: Math.min(...values),
        max: Math.max(...values)
      };
    });
    const normalizedItems = computed$2(
      () => props.items.filter((item) => Number.isFinite(item.value) && item.ratioPct > 0).map((item) => ({
        ...item,
        backgroundColor: colorForReturn(item.returnPct, returnRange.value),
        weight: Math.max(1, Math.round(item.ratioPct * 100))
      }))
    );
    const returnRangeLabel = computed$2(() => {
      if (!returnRange.value.hasRange) {
        return null;
      }
      return `${formatSignedPercent(returnRange.value.min)} ~ ${formatSignedPercent(returnRange.value.max)}`;
    });
    const expanded = ref$2(true);
    const infoOpen = ref$2(false);
    const topItem = computed$2(() => normalizedItems.value[0] ?? null);
    const collapsedSummary = computed$2(() => {
      if (!topItem.value) return "No holdings allocation data.";
      return `Top block: ${topItem.value.label} ${formatPercent(topItem.value.ratioPct)}`;
    });
    function loadExpandedState() {
      if (typeof window === "undefined" || !props.storageKey) return;
      const raw = window.localStorage.getItem(props.storageKey);
      if (raw === "1") expanded.value = true;
      if (raw === "0") expanded.value = false;
    }
    function formatCurrency(value, currency) {
      return new Intl.NumberFormat("ko-KR", {
        style: "currency",
        currency,
        maximumFractionDigits: 0
      }).format(value || 0);
    }
    function formatPercent(value) {
      return `${value.toFixed(2)}%`;
    }
    function formatSignedPercent(value) {
      if (value == null || !Number.isFinite(value)) return "-";
      return `${value >= 0 ? "+" : ""}${value.toFixed(2)}%`;
    }
    watch$2(expanded, (value) => {
      if (typeof window === "undefined" || !props.storageKey) return;
      window.localStorage.setItem(props.storageKey, value ? "1" : "0");
    });
    onMounted$2(() => {
      loadExpandedState();
    });
    return (_ctx, _cache) => {
      return _openBlock$2(), _createElementBlock$2("article", _hoisted_1$2, [
        _createElementVNode$2("div", _hoisted_2$2, [
          _createElementVNode$2("div", null, [
            _createElementVNode$2("div", _hoisted_3$2, [
              _createElementVNode$2("h3", _hoisted_4$2, _toDisplayString$2(__props.title), 1),
              _createElementVNode$2("button", {
                type: "button",
                class: "inline-flex h-7 w-7 items-center justify-center rounded-full border border-slate-300 text-xs font-semibold text-slate-700 transition-colors hover:bg-slate-100 dark:border-slate-700 dark:text-slate-200 dark:hover:bg-slate-800",
                "aria-pressed": infoOpen.value,
                "aria-label": infoOpen.value ? "Hide treemap info" : "Show treemap info",
                onClick: _cache[0] || (_cache[0] = ($event) => infoOpen.value = !infoOpen.value)
              }, " i ", 8, _hoisted_5$2)
            ]),
            __props.subtitle ? (_openBlock$2(), _createElementBlock$2("p", _hoisted_6$2, _toDisplayString$2(__props.subtitle), 1)) : _createCommentVNode$2("", true)
          ]),
          _createElementVNode$2("div", _hoisted_7$2, [
            _createElementVNode$2("span", _hoisted_8$2, _toDisplayString$2(__props.currency), 1),
            _createElementVNode$2("button", {
              type: "button",
              class: "rounded-xl border border-slate-300 px-3 py-2 text-sm font-semibold text-slate-700 transition-colors hover:bg-slate-100 dark:border-slate-700 dark:text-slate-200 dark:hover:bg-slate-800",
              onClick: _cache[1] || (_cache[1] = ($event) => expanded.value = !expanded.value)
            }, _toDisplayString$2(expanded.value ? "Collapse" : "Expand"), 1)
          ])
        ]),
        infoOpen.value ? (_openBlock$2(), _createElementBlock$2("div", _hoisted_9$2, [..._cache[2] || (_cache[2] = [
          _createElementVNode$2("p", null, "Treemap blocks are sized by allocation ratio, so bigger blocks represent larger positions.", -1),
          _createElementVNode$2("p", { class: "mt-1" }, "Color represents return tone, which helps us spot large winners and laggards at a glance.", -1)
        ])])) : _createCommentVNode$2("", true),
        expanded.value && __props.loading ? (_openBlock$2(), _createElementBlock$2("div", _hoisted_10$2, " Loading... ")) : expanded.value && __props.error ? (_openBlock$2(), _createElementBlock$2("div", _hoisted_11$2, _toDisplayString$2(__props.error), 1)) : expanded.value && normalizedItems.value.length === 0 ? (_openBlock$2(), _createElementBlock$2("div", _hoisted_12$2, " No holdings allocation data. ")) : !expanded.value ? (_openBlock$2(), _createElementBlock$2("div", _hoisted_13$2, [
          _cache[3] || (_cache[3] = _createElementVNode$2("p", { class: "font-medium text-slate-700 dark:text-slate-200" }, "Collapsed. Click Expand to preview the latest treemap.", -1)),
          _createElementVNode$2("p", _hoisted_14$2, _toDisplayString$2(collapsedSummary.value), 1),
          returnRangeLabel.value ? (_openBlock$2(), _createElementBlock$2("p", _hoisted_15$2, "Return range: " + _toDisplayString$2(returnRangeLabel.value), 1)) : _createCommentVNode$2("", true)
        ])) : (_openBlock$2(), _createElementBlock$2("div", _hoisted_16$2, [
          _createElementVNode$2("div", _hoisted_17$2, [
            (_openBlock$2(true), _createElementBlock$2(_Fragment$1, null, _renderList$1(normalizedItems.value, (item) => {
              return _openBlock$2(), _createElementBlock$2("div", {
                key: item.key,
                class: "flex min-h-[64px] min-w-[110px] flex-col justify-between rounded-lg p-2 text-xs text-white",
                style: _normalizeStyle$2({ flexGrow: item.weight, flexBasis: `${Math.max(18, item.ratioPct)}%`, backgroundColor: item.backgroundColor })
              }, [
                _createElementVNode$2("p", _hoisted_18$2, _toDisplayString$2(item.label), 1),
                _createElementVNode$2("div", _hoisted_19$2, [
                  _createElementVNode$2("p", null, [
                    _createTextVNode$2(_toDisplayString$2(formatPercent(item.ratioPct)) + " ", 1),
                    item.returnPct != null ? (_openBlock$2(), _createElementBlock$2(_Fragment$1, { key: 0 }, [
                      _createTextVNode$2("| " + _toDisplayString$2(formatSignedPercent(item.returnPct)), 1)
                    ], 64)) : _createCommentVNode$2("", true)
                  ]),
                  _createElementVNode$2("p", {
                    style: _normalizeStyle$2(props.maskAmounts ? { filter: "blur(6px)" } : void 0)
                  }, _toDisplayString$2(formatCurrency(item.value, __props.currency)), 5)
                ])
              ], 4);
            }), 128))
          ]),
          _cache[4] || (_cache[4] = _createElementVNode$2("p", { class: "text-[11px] text-slate-500 dark:text-slate-400" }, " Treemap blocks are sized by allocation ratio. Color represents return (green gain, red loss). ", -1)),
          returnRangeLabel.value ? (_openBlock$2(), _createElementBlock$2("p", _hoisted_20$2, " Return range: " + _toDisplayString$2(returnRangeLabel.value), 1)) : _createCommentVNode$2("", true)
        ]))
      ]);
    };
  }
});

const {defineComponent:_defineComponent$1} = await importShared('vue');

const {toDisplayString:_toDisplayString$1,createElementVNode:_createElementVNode$1,openBlock:_openBlock$1,createElementBlock:_createElementBlock$1,createCommentVNode:_createCommentVNode$1,normalizeStyle:_normalizeStyle$1,createTextVNode:_createTextVNode$1,normalizeClass:_normalizeClass$1} = await importShared('vue');

const _hoisted_1$1 = { class: "rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900" };
const _hoisted_2$1 = { class: "flex items-start justify-between gap-2" };
const _hoisted_3$1 = { class: "flex items-center gap-2" };
const _hoisted_4$1 = { class: "text-sm font-semibold text-slate-900 dark:text-slate-100" };
const _hoisted_5$1 = ["aria-pressed", "aria-label"];
const _hoisted_6$1 = {
  key: 0,
  class: "mt-0.5 text-xs text-slate-500 dark:text-slate-400"
};
const _hoisted_7$1 = { class: "flex items-center gap-2" };
const _hoisted_8$1 = { class: "rounded-full bg-slate-100 px-2 py-0.5 text-[11px] text-slate-600 dark:bg-slate-800 dark:text-slate-300" };
const _hoisted_9$1 = {
  key: 0,
  class: "mt-4 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-xs text-slate-600 dark:border-slate-700 dark:bg-slate-800/70 dark:text-slate-300"
};
const _hoisted_10$1 = {
  key: 1,
  class: "mt-4 rounded-xl border border-slate-200 bg-slate-50 px-3 py-3 text-sm text-slate-600 dark:border-slate-700 dark:bg-slate-800/70 dark:text-slate-300"
};
const _hoisted_11$1 = {
  key: 2,
  class: "mt-4 space-y-3 text-sm"
};
const _hoisted_12$1 = { class: "rounded-xl bg-slate-50 px-3 py-2 dark:bg-slate-800" };
const _hoisted_13$1 = { class: "font-semibold text-slate-800 dark:text-slate-100" };
const _hoisted_14$1 = { class: "mt-0.5 text-xs text-slate-500 dark:text-slate-400" };
const _hoisted_15$1 = { class: "rounded-xl bg-slate-50 px-3 py-2 dark:bg-slate-800" };
const _hoisted_16$1 = { class: "font-semibold text-slate-800 dark:text-slate-100" };
const _hoisted_17$1 = { class: "rounded-xl bg-slate-50 px-3 py-2 dark:bg-slate-800" };
const _hoisted_18$1 = { class: "font-semibold text-slate-800 dark:text-slate-100" };
const _hoisted_19$1 = { class: "mt-0.5 text-xs text-slate-500 dark:text-slate-400" };
const _hoisted_20$1 = {
  key: 3,
  class: "mt-3 text-[11px] text-slate-500 dark:text-slate-400"
};
const {computed: computed$1,onMounted: onMounted$1,ref: ref$1,watch: watch$1} = await importShared('vue');

const _sfc_main$1 = /* @__PURE__ */ _defineComponent$1({
  __name: "KpiSummaryCard",
  props: {
    currency: {},
    grossAssetsTotal: {},
    liabilitiesTotal: {},
    netAssetsTotal: {},
    investedPrincipalTotal: {},
    principalMinusDebtTotal: {},
    grossReturnPct: { default: null },
    netReturnPct: { default: null },
    grossProfitTotal: {},
    netProfitTotal: {},
    asOf: { default: "" },
    title: { default: "KPI Summary" },
    subtitle: { default: "" },
    maskAmounts: { type: Boolean, default: false },
    storageKey: { default: "" }
  },
  setup(__props) {
    const props = __props;
    function formatCurrency(value, currency) {
      return new Intl.NumberFormat("ko-KR", {
        style: "currency",
        currency,
        maximumFractionDigits: 0
      }).format(value);
    }
    function formatSignedCurrency(value, currency) {
      const abs = formatCurrency(Math.abs(value), currency);
      if (value > 0) return `+${abs}`;
      if (value < 0) return `-${abs}`;
      return abs;
    }
    function formatSignedPercent(value) {
      if (value == null || !Number.isFinite(value)) return "-";
      const num = Number(value);
      return `${num >= 0 ? "+" : ""}${num.toFixed(2)}%`;
    }
    function toneClass(value) {
      if (value == null || !Number.isFinite(value)) return "text-slate-400";
      if (value > 0) return "text-emerald-400";
      if (value < 0) return "text-rose-400";
      return "text-slate-300";
    }
    const expanded = ref$1(true);
    const infoOpen = ref$1(false);
    const collapsedSummary = computed$1(
      () => `Gross ${formatCurrency(props.grossAssetsTotal, props.currency)} · Net ${formatCurrency(props.netAssetsTotal, props.currency)}`
    );
    function loadExpandedState() {
      if (typeof window === "undefined" || !props.storageKey) return;
      const raw = window.localStorage.getItem(props.storageKey);
      if (raw === "1") expanded.value = true;
      if (raw === "0") expanded.value = false;
    }
    watch$1(expanded, (value) => {
      if (typeof window === "undefined" || !props.storageKey) return;
      window.localStorage.setItem(props.storageKey, value ? "1" : "0");
    });
    onMounted$1(() => {
      loadExpandedState();
    });
    return (_ctx, _cache) => {
      return _openBlock$1(), _createElementBlock$1("article", _hoisted_1$1, [
        _createElementVNode$1("div", _hoisted_2$1, [
          _createElementVNode$1("div", null, [
            _createElementVNode$1("div", _hoisted_3$1, [
              _createElementVNode$1("h3", _hoisted_4$1, _toDisplayString$1(__props.title), 1),
              _createElementVNode$1("button", {
                type: "button",
                class: "inline-flex h-7 w-7 items-center justify-center rounded-full border border-slate-300 text-xs font-semibold text-slate-700 transition-colors hover:bg-slate-100 dark:border-slate-700 dark:text-slate-200 dark:hover:bg-slate-800",
                "aria-pressed": infoOpen.value,
                "aria-label": infoOpen.value ? "Hide KPI Summary info" : "Show KPI Summary info",
                onClick: _cache[0] || (_cache[0] = ($event) => infoOpen.value = !infoOpen.value)
              }, " i ", 8, _hoisted_5$1)
            ]),
            __props.subtitle ? (_openBlock$1(), _createElementBlock$1("p", _hoisted_6$1, _toDisplayString$1(__props.subtitle), 1)) : _createCommentVNode$1("", true)
          ]),
          _createElementVNode$1("div", _hoisted_7$1, [
            _createElementVNode$1("span", _hoisted_8$1, _toDisplayString$1(__props.currency), 1),
            _createElementVNode$1("button", {
              type: "button",
              class: "rounded-xl border border-slate-300 px-3 py-2 text-sm font-semibold text-slate-700 transition-colors hover:bg-slate-100 dark:border-slate-700 dark:text-slate-200 dark:hover:bg-slate-800",
              onClick: _cache[1] || (_cache[1] = ($event) => expanded.value = !expanded.value)
            }, _toDisplayString$1(expanded.value ? "Collapse" : "Expand"), 1)
          ])
        ]),
        infoOpen.value ? (_openBlock$1(), _createElementBlock$1("div", _hoisted_9$1, [..._cache[2] || (_cache[2] = [
          _createElementVNode$1("p", null, "KPI Summary shows the latest Gross, Liabilities, and Net totals for the current scope.", -1),
          _createElementVNode$1("p", { class: "mt-1" }, "Gross is compared to invested principal, and Net is compared to debt-adjusted principal.", -1)
        ])])) : _createCommentVNode$1("", true),
        !expanded.value ? (_openBlock$1(), _createElementBlock$1("div", _hoisted_10$1, [
          _cache[3] || (_cache[3] = _createElementVNode$1("p", { class: "font-medium text-slate-700 dark:text-slate-200" }, "Collapsed. Click Expand to preview the latest KPI summary.", -1)),
          _createElementVNode$1("p", {
            class: "mt-1",
            style: _normalizeStyle$1(props.maskAmounts ? { filter: "blur(6px)" } : void 0)
          }, _toDisplayString$1(collapsedSummary.value), 5)
        ])) : (_openBlock$1(), _createElementBlock$1("div", _hoisted_11$1, [
          _createElementVNode$1("div", _hoisted_12$1, [
            _createElementVNode$1("p", _hoisted_13$1, [
              _cache[5] || (_cache[5] = _createTextVNode$1(" Gross: ", -1)),
              _createElementVNode$1("span", {
                style: _normalizeStyle$1(props.maskAmounts ? { filter: "blur(6px)" } : void 0)
              }, _toDisplayString$1(formatCurrency(__props.grossAssetsTotal, __props.currency)), 5),
              _createElementVNode$1("span", {
                class: _normalizeClass$1(toneClass(__props.grossReturnPct))
              }, [
                _createTextVNode$1(" ( " + _toDisplayString$1(formatSignedPercent(__props.grossReturnPct)) + ", ", 1),
                _createElementVNode$1("span", {
                  style: _normalizeStyle$1(props.maskAmounts ? { filter: "blur(6px)" } : void 0)
                }, _toDisplayString$1(formatSignedCurrency(__props.grossProfitTotal, __props.currency)), 5),
                _cache[4] || (_cache[4] = _createTextVNode$1(" ) ", -1))
              ], 2)
            ]),
            _createElementVNode$1("p", _hoisted_14$1, [
              _cache[6] || (_cache[6] = _createTextVNode$1(" vs invested principal ", -1)),
              _createElementVNode$1("span", {
                style: _normalizeStyle$1(props.maskAmounts ? { filter: "blur(6px)" } : void 0)
              }, " (" + _toDisplayString$1(formatCurrency(__props.investedPrincipalTotal, __props.currency)) + ") ", 5)
            ])
          ]),
          _createElementVNode$1("div", _hoisted_15$1, [
            _createElementVNode$1("p", _hoisted_16$1, [
              _cache[7] || (_cache[7] = _createTextVNode$1(" Liabilities: ", -1)),
              _createElementVNode$1("span", {
                style: _normalizeStyle$1(props.maskAmounts ? { filter: "blur(6px)" } : void 0)
              }, _toDisplayString$1(formatCurrency(__props.liabilitiesTotal, __props.currency)), 5)
            ])
          ]),
          _createElementVNode$1("div", _hoisted_17$1, [
            _createElementVNode$1("p", _hoisted_18$1, [
              _cache[9] || (_cache[9] = _createTextVNode$1(" Net: ", -1)),
              _createElementVNode$1("span", {
                style: _normalizeStyle$1(props.maskAmounts ? { filter: "blur(6px)" } : void 0)
              }, _toDisplayString$1(formatCurrency(__props.netAssetsTotal, __props.currency)), 5),
              _createElementVNode$1("span", {
                class: _normalizeClass$1(toneClass(__props.netReturnPct))
              }, [
                _createTextVNode$1(" ( " + _toDisplayString$1(formatSignedPercent(__props.netReturnPct)) + ", ", 1),
                _createElementVNode$1("span", {
                  style: _normalizeStyle$1(props.maskAmounts ? { filter: "blur(6px)" } : void 0)
                }, _toDisplayString$1(formatSignedCurrency(__props.netProfitTotal, __props.currency)), 5),
                _cache[8] || (_cache[8] = _createTextVNode$1(" ) ", -1))
              ], 2)
            ]),
            _createElementVNode$1("p", _hoisted_19$1, [
              _cache[10] || (_cache[10] = _createTextVNode$1(" vs debt-adjusted principal ", -1)),
              _createElementVNode$1("span", {
                style: _normalizeStyle$1(props.maskAmounts ? { filter: "blur(6px)" } : void 0)
              }, " (" + _toDisplayString$1(formatCurrency(__props.principalMinusDebtTotal, __props.currency)) + ") ", 5)
            ])
          ])
        ])),
        __props.asOf ? (_openBlock$1(), _createElementBlock$1("p", _hoisted_20$1, "as_of: " + _toDisplayString$1(__props.asOf), 1)) : _createCommentVNode$1("", true)
      ]);
    };
  }
});

const {defineComponent:_defineComponent} = await importShared('vue');

const {toDisplayString:_toDisplayString,createElementVNode:_createElementVNode,openBlock:_openBlock,createElementBlock:_createElementBlock,createCommentVNode:_createCommentVNode,normalizeStyle:_normalizeStyle,createTextVNode:_createTextVNode,renderList:_renderList,Fragment:_Fragment,unref:_unref,normalizeClass:_normalizeClass} = await importShared('vue');

const _hoisted_1 = { class: "rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900" };
const _hoisted_2 = { class: "flex items-start justify-between gap-2" };
const _hoisted_3 = { class: "flex items-center gap-2" };
const _hoisted_4 = { class: "text-sm font-semibold text-slate-900 dark:text-slate-100" };
const _hoisted_5 = ["aria-pressed", "aria-label"];
const _hoisted_6 = {
  key: 0,
  class: "mt-0.5 text-xs text-slate-500 dark:text-slate-400"
};
const _hoisted_7 = { class: "flex items-center gap-2" };
const _hoisted_8 = { class: "rounded-full bg-slate-100 px-2 py-0.5 text-[11px] text-slate-600 dark:bg-slate-800 dark:text-slate-300" };
const _hoisted_9 = {
  key: 0,
  class: "mt-4 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-xs text-slate-600 dark:border-slate-700 dark:bg-slate-800/70 dark:text-slate-300"
};
const _hoisted_10 = {
  key: 1,
  class: "mt-4 rounded-xl border border-slate-200 bg-slate-50 px-3 py-3 text-sm text-slate-600 dark:border-slate-700 dark:bg-slate-800/70 dark:text-slate-300"
};
const _hoisted_11 = {
  key: 2,
  class: "mt-3 rounded-xl bg-slate-50 p-3 text-sm text-slate-500 dark:bg-slate-800 dark:text-slate-300"
};
const _hoisted_12 = {
  key: 3,
  class: "mt-3 max-h-[24rem] overflow-auto rounded-xl border border-slate-200 dark:border-slate-700"
};
const _hoisted_13 = { class: "min-w-[860px] text-left text-xs" };
const _hoisted_14 = { class: "bg-slate-50 text-slate-600 dark:bg-slate-800 dark:text-slate-300" };
const _hoisted_15 = { class: "sticky-col-head sticky-col-width sticky left-0 z-20 bg-slate-50 px-3 py-2 dark:bg-slate-800" };
const _hoisted_16 = { class: "text-[11px] opacity-80" };
const _hoisted_17 = { class: "px-3 py-2 text-right" };
const _hoisted_18 = { class: "text-[11px] opacity-80" };
const _hoisted_19 = { class: "px-3 py-2 text-right" };
const _hoisted_20 = { class: "text-[11px] opacity-80" };
const _hoisted_21 = { class: "px-3 py-2 text-right" };
const _hoisted_22 = { class: "text-[11px] opacity-80" };
const _hoisted_23 = { class: "px-3 py-2 text-right" };
const _hoisted_24 = { class: "text-[11px] opacity-80" };
const _hoisted_25 = { class: "sticky-col-cell sticky-col-width sticky left-0 z-10 bg-white px-3 py-2 dark:bg-slate-900" };
const _hoisted_26 = ["title"];
const _hoisted_27 = ["title"];
const _hoisted_28 = { class: "px-3 py-2 text-right font-semibold text-slate-900 dark:text-slate-100" };
const _hoisted_29 = { class: "px-3 py-2 text-right text-slate-700 dark:text-slate-300" };
const {computed,onMounted,ref,watch} = await importShared('vue');

const {storeToRefs} = await importShared('pinia');
const _sfc_main = /* @__PURE__ */ _defineComponent({
  __name: "KpiPortfolioSummaryCard",
  props: {
    currency: {},
    portfolios: {},
    maskAmounts: { type: Boolean, default: false },
    title: { default: "KPI Portfolios" },
    subtitle: { default: "" },
    useNetBasis: { type: Boolean, default: false },
    storageKey: { default: "" }
  },
  setup(__props) {
    const props = __props;
    const uiStore = useUiStore();
    const { nameClampEnabled } = storeToRefs(uiStore);
    const expanded = ref(true);
    const infoOpen = ref(false);
    function toNumber(value) {
      if (value == null) return 0;
      const num = typeof value === "number" ? value : Number(value);
      return Number.isFinite(num) ? num : 0;
    }
    function formatCurrency(value, currency) {
      return new Intl.NumberFormat("ko-KR", {
        style: "currency",
        currency,
        maximumFractionDigits: 0
      }).format(value);
    }
    function formatSignedCurrency(value, currency) {
      const absText = formatCurrency(Math.abs(value), currency);
      if (value > 0) return `+${absText}`;
      if (value < 0) return `-${absText}`;
      return absText;
    }
    function formatPercent(value) {
      if (value == null || !Number.isFinite(value)) return "-";
      return `${value >= 0 ? "+" : ""}${value.toFixed(2)}%`;
    }
    function signedClass(value) {
      if (value >= 0) return "text-emerald-500 dark:text-emerald-400";
      return "text-rose-500 dark:text-rose-400";
    }
    function netContribution(row) {
      const explicitNet = row.net_contribution_total;
      if (explicitNet != null) {
        return toNumber(explicitNet);
      }
      return toNumber(row.cumulative_deposit_amount) - toNumber(row.cumulative_withdrawal_amount);
    }
    function currentValue(row) {
      if (props.useNetBasis) {
        return toNumber(row.net_assets_total);
      }
      return toNumber(row.gross_assets_total);
    }
    function principalValue(row) {
      if (props.useNetBasis) {
        return toNumber(row.debt_adjusted_principal_total ?? row.principal_minus_debt_total);
      }
      return netContribution(row);
    }
    function profitValue(row) {
      if (props.useNetBasis) {
        const explicit = row.net_assets_profit_total;
        if (explicit != null) return toNumber(explicit);
        return currentValue(row) - principalValue(row);
      }
      return toNumber(row.portfolio_profit_total ?? row.total_pnl_amount);
    }
    function returnValue(row) {
      if (props.useNetBasis) {
        const explicit = row.net_assets_return_pct;
        if (explicit != null) return toNumber(explicit);
        const base = principalValue(row);
        if (base > 0) {
          return (currentValue(row) - base) / base * 100;
        }
        return 0;
      }
      return toNumber(row.total_return_pct ?? null);
    }
    const currentColumnLabel = computed(() => props.useNetBasis ? "Current (Net)" : "Current Value");
    const principalColumnLabel = computed(() => props.useNetBasis ? "Debt-Adjusted Principal" : "Principal");
    const profitColumnLabel = computed(() => props.useNetBasis ? "Net Profit" : "Profit");
    const returnColumnLabel = computed(() => props.useNetBasis ? "Net Return" : "Return");
    const sortBy = ref("current");
    const sortOrder = ref("desc");
    function toggleSort(key) {
      if (sortBy.value === key) {
        sortOrder.value = sortOrder.value === "asc" ? "desc" : "asc";
        return;
      }
      sortBy.value = key;
      sortOrder.value = key === "portfolio" ? "asc" : "desc";
    }
    function sortIndicator(key) {
      if (sortBy.value !== key) return "↕";
      return sortOrder.value === "asc" ? "↑" : "↓";
    }
    const rows = computed(() => {
      const base = [...props.portfolios];
      base.sort((a, b) => {
        if (sortBy.value === "portfolio") {
          const byName = (a.name || "").localeCompare(b.name || "", "ko");
          if (byName !== 0) return byName;
          return Number(a.id) - Number(b.id);
        }
        if (sortBy.value === "current") {
          return currentValue(a) - currentValue(b);
        }
        if (sortBy.value === "principal") {
          return principalValue(a) - principalValue(b);
        }
        if (sortBy.value === "profit") {
          return profitValue(a) - profitValue(b);
        }
        return returnValue(a) - returnValue(b);
      });
      if (sortOrder.value === "desc") {
        base.reverse();
      }
      return base;
    });
    const collapsedSummary = computed(() => {
      const topRow = rows.value[0];
      if (!topRow) return "No portfolio KPI data.";
      return `Top portfolio · ${topRow.name} · ${formatCurrency(currentValue(topRow), topRow.base_currency || props.currency)}`;
    });
    function loadExpandedState() {
      if (typeof window === "undefined" || !props.storageKey) return;
      const raw = window.localStorage.getItem(props.storageKey);
      if (raw === "1") expanded.value = true;
      if (raw === "0") expanded.value = false;
    }
    watch(expanded, (value) => {
      if (typeof window === "undefined" || !props.storageKey) return;
      window.localStorage.setItem(props.storageKey, value ? "1" : "0");
    });
    onMounted(() => {
      if (typeof uiStore.init === "function") {
        uiStore.init();
      }
      loadExpandedState();
    });
    return (_ctx, _cache) => {
      return _openBlock(), _createElementBlock("article", _hoisted_1, [
        _createElementVNode("div", _hoisted_2, [
          _createElementVNode("div", null, [
            _createElementVNode("div", _hoisted_3, [
              _createElementVNode("h3", _hoisted_4, _toDisplayString(__props.title), 1),
              _createElementVNode("button", {
                type: "button",
                class: "inline-flex h-7 w-7 items-center justify-center rounded-full border border-slate-300 text-xs font-semibold text-slate-700 transition-colors hover:bg-slate-100 dark:border-slate-700 dark:text-slate-200 dark:hover:bg-slate-800",
                "aria-pressed": infoOpen.value,
                "aria-label": infoOpen.value ? "Hide KPI Portfolios info" : "Show KPI Portfolios info",
                onClick: _cache[0] || (_cache[0] = ($event) => infoOpen.value = !infoOpen.value)
              }, " i ", 8, _hoisted_5)
            ]),
            __props.subtitle ? (_openBlock(), _createElementBlock("p", _hoisted_6, _toDisplayString(__props.subtitle), 1)) : _createCommentVNode("", true)
          ]),
          _createElementVNode("div", _hoisted_7, [
            _createElementVNode("span", _hoisted_8, _toDisplayString(__props.currency), 1),
            _createElementVNode("button", {
              type: "button",
              class: "rounded-xl border border-slate-300 px-3 py-2 text-sm font-semibold text-slate-700 transition-colors hover:bg-slate-100 dark:border-slate-700 dark:text-slate-200 dark:hover:bg-slate-800",
              onClick: _cache[1] || (_cache[1] = ($event) => expanded.value = !expanded.value)
            }, _toDisplayString(expanded.value ? "Collapse" : "Expand"), 1)
          ])
        ]),
        infoOpen.value ? (_openBlock(), _createElementBlock("div", _hoisted_9, [..._cache[7] || (_cache[7] = [
          _createElementVNode("p", null, "KPI Portfolios compares current value, principal, profit, and return across portfolios.", -1),
          _createElementVNode("p", { class: "mt-1" }, "When Net basis is enabled, current, principal, profit, and return switch to debt-adjusted portfolio metrics.", -1)
        ])])) : _createCommentVNode("", true),
        !expanded.value ? (_openBlock(), _createElementBlock("div", _hoisted_10, [
          _cache[8] || (_cache[8] = _createElementVNode("p", { class: "font-medium text-slate-700 dark:text-slate-200" }, "Collapsed. Click Expand to preview KPI portfolios.", -1)),
          _createElementVNode("p", {
            class: "mt-1",
            style: _normalizeStyle(props.maskAmounts ? { filter: "blur(6px)" } : void 0)
          }, _toDisplayString(collapsedSummary.value), 5)
        ])) : rows.value.length === 0 ? (_openBlock(), _createElementBlock("div", _hoisted_11, " No portfolio KPI data. ")) : (_openBlock(), _createElementBlock("div", _hoisted_12, [
          _createElementVNode("table", _hoisted_13, [
            _createElementVNode("thead", _hoisted_14, [
              _createElementVNode("tr", null, [
                _createElementVNode("th", _hoisted_15, [
                  _createElementVNode("button", {
                    type: "button",
                    class: "inline-flex items-center gap-1 font-semibold text-inherit hover:text-slate-900 dark:hover:text-slate-100",
                    onClick: _cache[2] || (_cache[2] = ($event) => toggleSort("portfolio"))
                  }, [
                    _cache[9] || (_cache[9] = _createTextVNode(" Portfolio ", -1)),
                    _createElementVNode("span", _hoisted_16, _toDisplayString(sortIndicator("portfolio")), 1)
                  ])
                ]),
                _createElementVNode("th", _hoisted_17, [
                  _createElementVNode("button", {
                    type: "button",
                    class: "inline-flex items-center gap-1 font-semibold text-inherit hover:text-slate-900 dark:hover:text-slate-100",
                    onClick: _cache[3] || (_cache[3] = ($event) => toggleSort("current"))
                  }, [
                    _createTextVNode(_toDisplayString(currentColumnLabel.value) + " ", 1),
                    _createElementVNode("span", _hoisted_18, _toDisplayString(sortIndicator("current")), 1)
                  ])
                ]),
                _createElementVNode("th", _hoisted_19, [
                  _createElementVNode("button", {
                    type: "button",
                    class: "inline-flex items-center gap-1 font-semibold text-inherit hover:text-slate-900 dark:hover:text-slate-100",
                    onClick: _cache[4] || (_cache[4] = ($event) => toggleSort("principal"))
                  }, [
                    _createTextVNode(_toDisplayString(principalColumnLabel.value) + " ", 1),
                    _createElementVNode("span", _hoisted_20, _toDisplayString(sortIndicator("principal")), 1)
                  ])
                ]),
                _createElementVNode("th", _hoisted_21, [
                  _createElementVNode("button", {
                    type: "button",
                    class: "inline-flex items-center gap-1 font-semibold text-inherit hover:text-slate-900 dark:hover:text-slate-100",
                    onClick: _cache[5] || (_cache[5] = ($event) => toggleSort("profit"))
                  }, [
                    _createTextVNode(_toDisplayString(profitColumnLabel.value) + " ", 1),
                    _createElementVNode("span", _hoisted_22, _toDisplayString(sortIndicator("profit")), 1)
                  ])
                ]),
                _createElementVNode("th", _hoisted_23, [
                  _createElementVNode("button", {
                    type: "button",
                    class: "inline-flex items-center gap-1 font-semibold text-inherit hover:text-slate-900 dark:hover:text-slate-100",
                    onClick: _cache[6] || (_cache[6] = ($event) => toggleSort("return"))
                  }, [
                    _createTextVNode(_toDisplayString(returnColumnLabel.value) + " ", 1),
                    _createElementVNode("span", _hoisted_24, _toDisplayString(sortIndicator("return")), 1)
                  ])
                ])
              ])
            ]),
            _createElementVNode("tbody", null, [
              (_openBlock(true), _createElementBlock(_Fragment, null, _renderList(rows.value, (row) => {
                return _openBlock(), _createElementBlock("tr", {
                  key: row.id,
                  class: "border-t border-slate-200 dark:border-slate-800"
                }, [
                  _createElementVNode("td", _hoisted_25, [
                    _createElementVNode("p", {
                      title: `#${row.id} ${row.name}`,
                      class: _normalizeClass(["font-semibold text-slate-900 dark:text-slate-100", _unref(nameClampEnabled) ? "name-clamp-2" : void 0])
                    }, " #" + _toDisplayString(row.id) + " " + _toDisplayString(row.name), 11, _hoisted_26),
                    _createElementVNode("p", {
                      title: row.type,
                      class: "text-[11px] text-slate-500 dark:text-slate-400"
                    }, _toDisplayString(row.type), 9, _hoisted_27)
                  ]),
                  _createElementVNode("td", _hoisted_28, [
                    _createElementVNode("span", {
                      style: _normalizeStyle(props.maskAmounts ? { filter: "blur(6px)" } : void 0)
                    }, _toDisplayString(formatCurrency(currentValue(row), row.base_currency || __props.currency)), 5)
                  ]),
                  _createElementVNode("td", _hoisted_29, [
                    _createElementVNode("span", {
                      style: _normalizeStyle(props.maskAmounts ? { filter: "blur(6px)" } : void 0)
                    }, _toDisplayString(formatCurrency(principalValue(row), row.base_currency || __props.currency)), 5)
                  ]),
                  _createElementVNode("td", {
                    class: _normalizeClass(["px-3 py-2 text-right font-semibold", signedClass(profitValue(row))])
                  }, [
                    _createElementVNode("span", {
                      style: _normalizeStyle(props.maskAmounts ? { filter: "blur(6px)" } : void 0)
                    }, _toDisplayString(formatSignedCurrency(profitValue(row), row.base_currency || __props.currency)), 5)
                  ], 2),
                  _createElementVNode("td", {
                    class: _normalizeClass(["px-3 py-2 text-right font-semibold", signedClass(returnValue(row))])
                  }, _toDisplayString(formatPercent(returnValue(row))), 3)
                ]);
              }), 128))
            ])
          ])
        ]))
      ]);
    };
  }
});

export { _sfc_main$1 as _, _sfc_main as a, _sfc_main$3 as b, _sfc_main$2 as c };
