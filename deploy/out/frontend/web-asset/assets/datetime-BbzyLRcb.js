...
function formatDateTimeSeoul(value) {
...
function seoulDateToUtcNaiveIso(value, endOfDay = false) {
  const trimmed = value.trim();
  if (!trimmed) return void 0;
  if (!/^\d{4}-\d{2}-\d{2}$/.test(trimmed)) return void 0;
  const hhmmss = endOfDay ? "23:59:59" : "00:00:00";
  const date = /* @__PURE__ */ new Date(`${trimmed}T${hhmmss}+09:00`);
...
function toDateTimeLocalSeoul(value) {
  const parsed = parseApiUtcDate(value);
  if (!parsed) return "";
