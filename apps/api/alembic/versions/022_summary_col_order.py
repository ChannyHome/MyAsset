"""022 reorder summary view columns by KPI priority

Revision ID: 022_summary_col_order
Revises: 021_portfolio_col_order
Create Date: 2026-02-16 23:58:00
"""

from alembic import op


revision = "022_summary_col_order"
down_revision = "021_portfolio_col_order"
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    ...
    op.execute(
        ...
        SELECT
            ...
            COALESCE(a.gross_assets_total, 0) - COALESCE(ls.liabilities_total_effective, 0) AS net_assets_total,
            (
                COALESCE(pt.invested_principal_total, 0)
                - COALESCE(ls.liabilities_total_effective, 0)
            ) AS principal_minus_debt_total,
            (
                (COALESCE(a.gross_assets_total, 0) - COALESCE(ls.liabilities_total_effective, 0))
            ...
            (
                COALESCE(a.gross_assets_total, 0)
        ...
        LEFT JOIN (
            SELECT
                owner_user_id,
                SUM(evaluated_amount) AS gross_assets_total,
